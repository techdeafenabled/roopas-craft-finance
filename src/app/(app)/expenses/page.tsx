"use client";
import { useEffect, useState, useCallback } from "react";
import { db } from "@/lib/db";
import { formatINR, formatDate } from "@/lib/format";
import EntryForm from "@/components/EntryForm";
import { Receipt } from "lucide-react";
import type { TransactionWithBanks } from "@/lib/types";

const CATEGORY_LABELS: Record<string, string> = {
  stall: "Stall Fee",
  fuel: "Fuel",
  food: "Food",
  travel: "Travel",
  salary: "Salary",
  other: "Other",
};

export default function ExpensesPage() {
  const [tab, setTab] = useState<"add" | "history">("add");
  const [expenses, setExpenses] = useState<TransactionWithBanks[]>([]);

  const loadExpenses = useCallback(async () => {
    const [txAll, txBanksAll, banks] = await Promise.all([
      db.transactions.where("type").equals("expense").reverse().sortBy("date"),
      db.transaction_banks.toArray(),
      db.banks.toArray(),
    ]);
    setExpenses(
      txAll.map((tx) => ({
        ...tx,
        transaction_banks: txBanksAll
          .filter((tb) => tb.transaction_id === tx.id)
          .map((tb) => ({ ...tb, bank: banks.find((b) => b.id === tb.bank_id)! })),
      }))
    );
  }, []);

  useEffect(() => {
    loadExpenses();
  }, [loadExpenses]);

  return (
    <div className="px-4 pt-6 flex flex-col gap-5">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-red-50 flex items-center justify-center">
          <Receipt size={20} className="text-expense" />
        </div>
        <h1 className="text-xl font-bold text-[var(--text-primary)]">Expenses</h1>
      </div>

      <div className="flex bg-[var(--off-white)] rounded-xl p-1">
        {(["add", "history"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-colors ${
              tab === t ? "bg-white text-[var(--text-primary)] shadow-sm" : "text-[var(--text-secondary)]"
            }`}
          >
            {t === "add" ? "Add Expense" : "History"}
          </button>
        ))}
      </div>

      {tab === "add" ? (
        <EntryForm type="expense" onSuccess={loadExpenses} />
      ) : (
        <div className="flex flex-col gap-2">
          {expenses.length === 0 ? (
            <div className="text-center py-12 text-[var(--text-secondary)] text-sm">
              No expenses recorded yet.
            </div>
          ) : (
            expenses.map((tx) => (
              <div key={tx.id} className="card flex items-center gap-3 py-3">
                <div className="w-9 h-9 rounded-xl bg-red-50 flex items-center justify-center">
                  <Receipt size={16} className="text-expense" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-expense">{formatINR(tx.amount)}</p>
                  <p className="text-xs text-[var(--text-secondary)] truncate">
                    {tx.expense_category ? CATEGORY_LABELS[tx.expense_category] : ""}
                    {tx.note ? ` · ${tx.note}` : ""}
                    {" · "}
                    {tx.transaction_banks.map((tb) => tb.bank?.name).join(" + ")}
                  </p>
                </div>
                <p className="text-xs text-[var(--text-secondary)] shrink-0">{formatDate(tx.date)}</p>
              </div>
            ))
          )}
        </div>
      )}
    </div>
  );
}
