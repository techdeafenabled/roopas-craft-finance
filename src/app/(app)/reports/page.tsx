...
const PIE_COLORS = ["#4f6228", "#c8a059", "#dc2626", "#d97706", "#16a34a", "#6b7280", "#8b5cf6", "#ec4899"];
      ...
      <div className="card">
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={monthlyProfit} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
            ...
            <YAxis tick={{ fontSize: 10 }} tickFormatter={(v) => `₹${(v / 1000).toFixed(0)}k`} />
            <Tooltip formatter={(v) => formatINR(Number(v))} />
            <Line type="monotone" dataKey="profit" stroke="#4f6228" strokeWidth={2.5} dot={{ r: 4, fill: "#4f6228" }} />
            <Line type="monotone" dataKey="sales" stroke="#16a34a" strokeWidth={1.5} strokeDasharray="5 5" dot={false} />
          </LineChart>
        </ResponsiveContainer>
        <div className="flex gap-4 mt-2 justify-center">
          <div className="flex items-center gap-1.5 text-xs text-[var(--text-secondary)]">
            <div className="w-3 h-0.5 rounded" style={{ background: "#4f6228" }} />
            Profit
          </div>
          <div className="flex items-center gap-1.5 text-xs text-[var(--text-secondary)]">
